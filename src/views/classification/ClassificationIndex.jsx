...
export default function ClassificationIndex() {
  ...
  const [classifications, setClassifications] = useState([]);  // Menyimpan data klasifikasi
  ...
  const fetchClassifications = useCallback(async () => {
    try {
      ...
      let url = `/classifications?size=${pageSize}&page=${currentPage}&sort=DESC`;
  ...
  const openModal = (action, classification = null) => {
    ...
    <div className="mt-2 mb-2">
      <div className="bg-white rounded-lg shadow-md px-4 py-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-3xl font-bold text-gray-800">Classification</h1>
          <button
        ...
        <div className="border border-gray-300 rounded-lg text-gray-800 px-4 py-6">
          <table className="min-w-full bg-white">
            <thead>
              <tr>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-1/14">#</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-1/4">Name</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-1/4">Table Name</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-1/4">Field Name</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-1/12" style={{ width: "150px" }}>Actions</th>
              </tr>
            </thead>
            <tbody>
                ...
                classifications.map((classification, index) => {
                  ...
                  return (
                    <tr key={classification.id}>
                      ...
                      <td className="border-b border-gray-300 px-4 py-2 text-left">{classification.name}</td>
                      <td className="border-b border-gray-300 px-4 py-2 text-left">{classification.table_name}</td>
                      <td className="border-b border-gray-300 px-4 py-2 text-left">{classification.field_name}</td>
                      <td className="border-b border-gray-300 px-4 py-2 text-left">
                        ...
                        <button
                          ...
                          onClick={() => openModal("detail", classification)}  // Menampilkan detail
                        >
                        ...
                        <button
                          ...
                          onClick={() => openModal("edit", classification)}  // Menampilkan form edit
                        >
                          <FontAwesomeIcon icon="fas fa-edit" />
                        </button>
                        <button
                          className="bg-red-600 text-white px-1 rounded hover:bg-red-700 transition duration-200"
                          onClick={() => openModal("delete", classification)}  // Menampilkan konfirmasi delete
                        >
        ...
        <ModalForm
          ...
          reloadClassifications={fetchClassifications}  // Memuat ulang data klasifikasi setelah aksi selesai
        />
      </div>
    </div>
    </>
