...
export default function DiscountIndex() {
  ...
  const [isOpenTringger, setIsOpenTringger] = useState(false); 


  const fetchDiscounts = useCallback(async () => {
    try {
      
      let url = `/discounts?size=${pageSize}&page=${currentPage}&sort=DESC`;
      ...
      setDiscounts(response.data.payload);
  ...
  const openModal = (action, discount = null) => {
    setModalAction(action); 
    setCurrentDiscount(discount); 
    setIsModalOpen(true);  // Buka modal
    setIsOpenTringger(true);
  };
    ...
    <div className="mt-2 mb-2">
      <div className="bg-white rounded-lg shadow-md px-4 py-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-3xl font-bold text-gray-800">Discount</h1>
          <button
        ...
        <div className="border border-gray-300 rounded-lg text-gray-800 px-4 py-6">
          <table className="min-w-full bg-white">
            <thead>
              <tr>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/14">#</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/7">Product</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/10">Discount</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/7">Start</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/7">Expired</th>
                <th className="border-b-2 border-gray-300 px-4 py-2 text-left w-2/4" style={{ width: "150px" }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {discounts.length > 0 ? (
                discounts.map((discount, index) => {
                  ...
                  return (
                    <tr key={discount.id}>
                      ...
                      <td className="border-b border-gray-300 px-4 py-2 text-left">{discount.product_name}</td>
                      <td className="border-b border-gray-300 px-4 py-2 text-left">{discount.discount_value} %</td>
                      <td className="border-b border-gray-300 px-4 py-2 text-left">
                        {new Date(discount.start_at).toLocaleDateString("id-ID", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                          hour: "2-digit",
                      ...
                      <td className="border-b border-gray-300 px-4 py-2 text-left">
                        {new Date(discount.expired_at).toLocaleDateString("id-ID", {
                          day: "2-digit",
                          month: "short",
                          year: "numeric",
                          hour: "2-digit",
                      ...
                      <td className="border-b border-gray-300 px-4 py-2 text-left">
                        ...
                        <button
                          ...
                          onClick={() => openModal("detail", discount)}  // Menampilkan detail
                        >
                        ...
                        <button
                          ...
                          onClick={() => openModal("edit", discount)}  // Menampilkan form edit
                        >
                          <FontAwesomeIcon icon="fas fa-edit" />
                        </button>
                        <button
                          className="bg-red-600 text-white px-1 rounded hover:bg-red-700 transition duration-200"
                          onClick={() => openModal("delete", discount)}  // Menampilkan konfirmasi delete
                        >
        ...
        {isOpenTringger && (
              ...
              reloadDiscounts={fetchDiscounts}  // Memuat ulang data klasifikasi setelah aksi selesai
            />
            )}
