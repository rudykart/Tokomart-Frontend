...
function Filter({ onFilterChange }) {  // Tambahkan prop onFilterChange
  ...
  const [role, setRole] = useState("");    // State untuk filter Table Name
  ...
  const searchKey = useMemo(() => {
    ...
    if (role) keyParts.push(`role=${role}`);
          ...
          >
            <div className="border border-gray-300 rounded-lg text-gray-800 px-4 py-6 mt-2 mb-2">
              <div className=" flex items-center">
                <label htmlFor="role" className="text-md text-gray-700 w-32">Role</label>
                <select
                  id="role"
                  value={role}
